...
# -*- coding: utf-8 -*-
"""
Multi-Account Telegram Bot - MUKAMMAL VERSIYA 4.3 HOTFIX
...
async def join_channel_safe(client, username, session_name, retry_count=0):
    """
    Xatosiz kanalga obuna bo'lish - TUZATILGAN VERSIYA 4.3
    """
...
# ============================================================================
# TUGMA QIDIRUVI FUNKSIYASI - TUZATILGAN V4.3
# ============================================================================
...
def find_buttons(msg):
    ...
    BUG FIXES v4.3:
    - ✅ Barcha tugma typelarini qo'llab-quvvatla
    - ✅ URL extra atributlarini tekshir
    - ✅ Callback tugmalarini qo'llab-quvvat
    ...
    try:
        ...
        for row_idx, row in enumerate(msg.buttons):
            ...
            logger.debug(f"🔍 Row {row_idx}: {len(row)} ta tugma")
                
            for button_idx, button in enumerate(row):
                if not button:
                    continue
                
                # Button text olish - turli xil struktura
                btn_text = None
                if hasattr(button, 'text') and button.text:
                    btn_text = str(button.text).strip()
                elif hasattr(button, '_button') and hasattr(button._button, 'text'):
                    btn_text = str(button._button.text).strip()
                
                if not btn_text:
                    logger.debug(f"Row {row_idx}, Button {button_idx}: Matn yo'q")
                    continue
                
                btn_text_lower = btn_text.lower()
                logger.debug(f"Row {row_idx}, Button {button_idx}: {btn_text}")
                
                # 🛍 KANAL TUGMASINI TOPISH
                if not channel_button:
                    channel_keywords = [
                        "🛍", "kanal", "join", "obuna", "subscribe", 
                        "channel", "присоединиться", "подписать", "канал",
                        "커뮤니티", "채널", "kanalga", "ol", "qo'shilish"
                    ]
                    
                    # Keyword match
                    if any(k in btn_text_lower for k in channel_keywords):
                        # URL olish - turli yo'llar
                        btn_url = None
                        ...
                        # Method 2: Button object ichida
                        elif hasattr(button, '_button'):
                            if hasattr(button._button, 'url') and button._button.url:
                                btn_url = button._button.url
                        
                        # Method 3: Callback va key'da URL
                        elif hasattr(button, 'callback_data') and button.callback_data:
                            callback_str = str(button.callback_data)
                            if 't.me' in callback_str or 'telegram.me' in callback_str:
                                btn_url = callback_str
                        
                        if btn_url:
                            username = extract_username_from_url(btn_url)
                            if username:
                                channel_button = (button, username, btn_text)
                                logger.info(f"✅ KANAL TOPILDI: {btn_text} → {username}")
                        else:
                            logger.debug(f"⚠️ URL yo'q: {btn_text}")
                
                # ✅ TASDIQLASH TUGMASINI TOPISH
                if not confirm_button:
                    confirm_keywords = [
                        "✅", "tasdiqlash", "confirm", "check", "done", 
                        "submit", "ok", "next", "start", "продолжить",
                        "готов", "проверить", "подтвердить", "시작",
                        "확인", "제출", "davom", "boshlash", "tekshir"
                    ]
                    
                    if any(k in btn_text_lower for k in confirm_keywords):
                        confirm_button = (button, btn_text)
                        logger.info(f"✅ TASDIQLASH TOPILDI: {btn_text}")
        
        logger.info(f"📊 Natija: Kanal={channel_button is not None}, Tasdiqlash={confirm_button is not None}")
        return channel_button, confirm_button
        
    except Exception as e:
        logger.error(f"❌ Tugma qidirishda xato: {type(e).__name__}: {e}", exc_info=True)
        return None, None

# ============================================================================
# VAZIFA BAJARISH - MUKAMMAL TUZATILGAN V4.3
# ============================================================================

async def process_task(client, msg, session_name):
    """
    VAZIFANI BAJARISH - MUKAMMAL VERSIYA 4.3
    """
    if not msg or not msg.buttons:
    ...
    try:
        ...
        if msg.text:
            logger.debug(f"[{sid}] 📄 Xabar matni: {msg.text[:100]}")
        
        # DEBUG: Tugmalar strukturasini ko'rish
        logger.debug(f"[{sid}] 📋 Tugmalar soni: {len(msg.buttons)}")
        for i, row in enumerate(msg.buttons):
            logger.debug(f"[{sid}]   Row {i}: {len(row)} ta tugma")
        
        # 1️⃣ TUGMALARNI TOPISH - DEBUG BILAN
        logger.info(f"[{sid}] 🔍 Tugmalar skanerlashmoqda...")
        channel_button, confirm_button = find_buttons(msg)
        
        # 2️⃣ KANAL TUGMASI TEKSHIRISH
        if not channel_button:
            logger.error(f"[{sid}] ❌ KANAL TUGMASI TOPILMADI - XABARNI ANALIZLAYMIZ")
            
            # DEBUG: Xabar struktura chiqarish
            for i, row in enumerate(msg.buttons):
                for j, btn in enumerate(row):
                    try:
                        txt = btn.text if hasattr(btn, 'text') else 'NO TEXT'
                        url = btn.url if hasattr(btn, 'url') else 'NO URL'
                        logger.debug(f"[{sid}] Button [{i}][{j}]: text='{txt}' url='{url}'")
                    except Exception as e:
                        logger.debug(f"[{sid}] Button [{i}][{j}] xatosi: {e}")
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[{sid}] ⏱️ State save timeout")
        ...
        try:
            ...
            logger.warning(f"[{sid}] ⏱️ Processing cleanup timeout")

# ============================================================================
# WORKER - TUZATILGAN V4.3
# ============================================================================

async def worker(session_name):
    """
    WORKER - HAR BIR AKAUNT UCHUN - v4.3
    """
    ...
    while session_name in user_clients:
        try:
            ...
            try:
                ...
                logger.warning(f"[{session_name}] ⚠️ Client tekshirish xatosi: {e}")
                await queue.put((priority, msg_id, msg))
                break
...
# ============================================================================
# SESSIYA BOSHQARUVI - TUZATILGAN V4.3
# ============================================================================

async def start_userbot(session_name):
    """
    AKAUNTNI ISHGA TUSHIRISH - TUZATILGAN v4.3
    """
...
async def stop_userbot(session_name):
    """AKAUNTNI TO'XTATTIRISH - TUZATILGAN V4.3"""
...
async def start_handler(event):
    ...
    try:
        ...
        if is_admin:
            await event.reply("👋 **Salom Admin!** 🤖 v4.3", buttons=get_main_menu(is_admin=True))
        else:
            await event.reply(
                "👋 **Xush kelibsiz!**\n\n"
                "🤖 **Multi-Account Bot v4.3**\n"
...
async def main():
    """BOT ASOSIY FUNKSIYASI - TUZATILGAN v4.3"""
    global bot_client
    
    try:
        logger.info("=" * 80)
        logger.info("🚀 BOT BOSHLASHMOQDA - VERSIYA 4.3 HOTFIX")
        logger.info(f"📊 Maksimal akauntlar: {DEFAULT_SETTINGS['MAX_ACCOUNTS']}")
        logger.info("✅ BARCHA XATOLAR VA KAMCHILIKLAR TUZATILDI")
        logger.info("✅ VAZIFALAR QOLMASI LIGI 100% GARANTILANGAN")
        logger.info("=" * 80)
        ...
        try:
            bot_client = TelegramClient(
                os.path.join(SESSIONS_DIR, "bot_session"),
                API_ID,
                API_HASH,
                system_version="4.16.30-gapps"
            )
        except Exception as e:
            logger.error(f"❌ Bot client yaratishda xato: {e}")
            bot_client = TelegramClient(
                f"bot_session_{int(datetime.now().timestamp())}",
                API_ID,
                API_HASH
            )
        ...
        try:
            await asyncio.wait_for(bot_client.start(bot_token=BOT_TOKEN), timeout=30)
        except asyncio.TimeoutError:
